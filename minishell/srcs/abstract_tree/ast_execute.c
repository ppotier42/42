...
/*   Updated: 2023/09/26 15:04:30 by ppotier          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ast_execute_built(t_node *root, t_env *envcpy)
{
	t_builtin	*content;
	int			status;
	int			out_fd;
	...
	if (root->pid == 0)
	{
		...
		status = content->func_pointer(content->args, envcpy);
		exit(status);
	}
	else if (root->pid == -1)
	{
		...
		ast_node_redirect(root);
		content = (t_builtin *) root->content;
		root->exit_status = content->func_pointer(content->args, envcpy);
		restore_fds(root, out_fd, in_fd);
	}
}

void	ast_execute(t_node *root, t_env *envcpy)
{
	if (!root)
		return ;
	if (root->type == AST_ROOT)
	{
		ast_execute(root->children[0], envcpy);
		return ;
	}
	if (root->type == AST_PIPE)
		handle_pipe(root);
	ast_execute(root->children[0], envcpy);
	ast_execute(root->children[1], envcpy);
	if (root->type == AST_CMD)
		ast_execute_cmd(root, envcpy);
	else if (root->type == AST_BUILTIN)
		ast_execute_built(root, envcpy);
