...
/*   Updated: 2023/09/26 14:45:51 by ppotier          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_access_status(char *path, char *cmd_name)
{
	...
	{
		ft_put_error(cmd_name, "No such file or directory");
		return (127);
	}
...
char	*get_cmd(char *cmd, t_env *envcpy)
{
	char	*command;
	char	*env_path;
	char	**path;
	...
	env_path = ms_getenv_cont("PATH", envcpy);
	if (!env_path)
		return (NULL);
...
void	ast_execute_cmd(t_node *root, t_env *envcpy)
{
	...
	full_path = get_cmd(cmd->cmd_name, envcpy);
	...
	if (root->pid == 0)
	{
		...
		execve(full_path, cmd->args, envcpy->env);
	}
	free (full_path);
}
